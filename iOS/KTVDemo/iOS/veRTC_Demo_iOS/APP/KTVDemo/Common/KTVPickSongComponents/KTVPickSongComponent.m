...
- (void)requestPickedSongList {
    __weak typeof(self) weakSelf = self;
    [KTVRTMManager requestPickedSongList:self.roomID block:^(RTMACKModel * _Nonnull model, NSArray<KTVSongModel *> * _Nonnull list) {
...
- (void)syncSongListStstus {
    for (KTVSongModel *model in self.onlineListView.dataArray) {
        ...
        for (KTVSongModel *pickedModel in self.pickedListView.dataArray) {
            if ([pickedModel.pickedUserID isEqualToString:[LocalUserComponents userModel].uid] &&
